...
\begin{ourresponse}
We thank the reviewer for raising this point, which was not properly addressed in the original manuscript. To address this, we added to the revised manuscript a systematic study examining how the choice of modulation depth affects reconstruction quality. The results of this study have been added to the Supplementary Materials section discussing the amplitude modulation.
...
\begin{quote}
    \subsection*{Matrix-based scattering compensation algorithms}
...
The CLASS algorithm \cite{kang17,kang2024implementation} iteratively decomposes the reflection matrix to recover both the object and the phase distortions.
In real space we can write
\(\mathbf R=\mathbf P_{\mathrm{det}}\mathbf O\mathbf P_{\mathrm{ill}}\),
a Toeplitz–Diagonal–Toeplitz (TDT) product; after a two‑dimensional Fourier transform this becomes the Diagonal–Toeplitz–Diagonal (DTD) form
...
\paragraph{Notation.}
$\phi_k$ denotes the illumination‑phase distortion at pixel $k$, so
\(\tilde{\mathbf P}_{\mathrm{ill}}
      =\operatorname{diag}(e^{i\phi_1},e^{i\phi_2},\dots)\);
$O_\ell$ is the $\ell$‑th Fourier coefficient of the object that appears
shifted along each column of the Toeplitz matrix \(\tilde{\mathbf O}\).
The algorithm operates under the assumption that both
\(\tilde{\mathbf P}_{\mathrm{ill}}\) and \(\tilde{\mathbf P}_{\mathrm{det}}\)
are phase‑only masks.

To illustrate the core idea, we present a \(3\times3\) toy example; the
full \(N\times N\) case follows by direct extension.

\textbf{1.~Toy \(3\times3\) matrices.}
Set
\[
\tilde{\mathbf P}_{\mathrm{ill}}
...
\tilde{\mathbf O}
  =\begin{bmatrix}
   ...
   \end{bmatrix},
\]
and let \(\tilde{\mathbf P}_{\mathrm{det}}\) have diagonal
\(\{e^{i\psi_1},e^{i\psi_2},e^{i\psi_3}\}\).
Equation~\eqref{eq:Rtilde_def} then yields the explicit \(3\times3\)
reflection matrix that we will manipulate in the following steps.


\textbf{2. Alternating correction approach:} The algorithm's core strategy is to alternately correct each distortion by temporarily ignoring one while focusing on the other. For example, to correct the illumination distortion $\tilde{\mathbf{P}}_{ill}$, we conceptually treat $\tilde{\mathbf{R}}$ as if it were just $\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill}$ (temporarily ignoring $\tilde{\mathbf{P}}_{det}$). For our 3×3 example, if we had $\tilde{\mathbf{O}}$ and $\tilde{\mathbf{P}}_{ill}$ the product would be:
\begin{equation}
\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill} = 
\begin{bmatrix}
O_1 e^{i\phi_1} & O_3 e^{i\phi_2} & O_2 e^{i\phi_3} \\
O_2 e^{i\phi_1} & O_1 e^{i\phi_2} & O_3 e^{i\phi_3} \\
O_3 e^{i\phi_1} & O_2 e^{i\phi_2} & O_1 e^{i\phi_3}
\end{bmatrix}
\end{equation}

\textbf{3. Column-shifting operation:} The key insight of CLASS is that, due to the Toeplitz structure of $\tilde{\mathbf{O}}$, shifting the columns of the matrix in a specific way aligns the object components across columns. This alignment makes the phase distortions more apparent. When we apply this column-shifting operation to our simplified matrix, we obtain:
\begin{equation}
(\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill})_{\text{shifted}} = 
\begin{bmatrix}
O_1 e^{i\phi_1} & O_1 e^{i\phi_2} & O_1 e^{i\phi_3} \\
O_2 e^{i\phi_1} & O_2 e^{i\phi_2} & O_2 e^{i\phi_3} \\
O_3 e^{i\phi_1} & O_3 e^{i\phi_2} & O_3 e^{i\phi_3}
\end{bmatrix}
\end{equation}

Notice how each row now contains the same object component ($O_1$, $O_2$, or $O_3$) multiplied by different phase factors. 

\textbf{4. Extracting phase information:} To extract the phase distortions, we perform two key operations:

a) First, we take the mean of each row of $(\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill})_{\text{shifted}}$ (averaging across columns):
\begin{equation}
\vec{T} = 
\begin{bmatrix}
O_1 M \\
...
\end{bmatrix}
\end{equation}
where $M = \frac{1}{3}(e^{i\phi_1} + e^{i\phi_2} + e^{i\phi_3}) = |M|e^{i\theta_M}$ and $\theta_M$ is a mean phase of $P_{ill}$.

b) Next, we multiply the transpose of $(\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill})_{\text{shifted}}$ by the conjugate of $\vec{T}$:
\begin{equation}
\vec{v} =
 (\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill})_{\text{shifted}}^T \vec{T}^*=
 \begin{bmatrix}
O_{1}e^{i\phi_1} & O_{2}e^{i\phi_1} & O_{3}e^{i\phi_1}\\
O_{1}e^{i\phi_2} & O_{2}e^{i\phi_2} & O_{3}e^{i\phi_2}\\
O_{1}e^{i\phi_3} & O_{2}e^{i\phi_3} & O_{3}e^{i\phi_3}
\end{bmatrix}
\begin{bmatrix}
O_{1}^{*}M^{*}\\[2pt]
O_{2}^{*}M^{*}\\[2pt]
...
\end{bmatrix}
\end{equation}
where $S = |O_1|^2 + |O_2|^2 + |O_3|^2 > 0$ is a positive scalar.

The phase angle of each element in $\vec{v}$ gives us:
\begin{equation}
\arg(\vec{v}) = 
\begin{bmatrix}
\phi_1 - \theta_M \\
...
This procedure determines the phases of $\tilde{\mathbf{P}}_{ill}$ up to a global offset $\theta_M$, which is physically irrelevant because only relative phases affect image reconstruction.

\textbf{5. Applying the illumination correction:} We create a diagonal correction matrix using the extracted phase information and multiply it with $\tilde{\mathbf{R}}$:
\begin{equation}
\tilde{\mathbf{P}}_{ill}^{\text{correction}} = \operatorname{diag}(e^{-i\arg(v_1)}, e^{-i\arg(v_2)}, e^{-i\arg(v_3)})
\end{equation}
...
\textbf{6. Detection distortion correction:} After correcting the illumination distortion, we switch focus to the detection distortion. To do this, we transpose the reflection matrix:
\begin{equation}
\tilde{\mathbf{R}}^T = \tilde{\mathbf{P}}_{ill}^T \tilde{\mathbf{O}}^T \tilde{\mathbf{P}}_{det}^T  
\end{equation}

Now the roles are switched, with $\tilde{\mathbf{P}}_{det}^T$ playing the role that $\tilde{\mathbf{P}}_{ill}$ played before. We apply the same shifting, correlation, and correction procedures to correct the detection distortion.

\textbf{7. Iterative refinement:} The illumination and detection correction steps are alternated until successive phase estimates no longer change appreciably; convergence of this loop under single‑scattering conditions is proven in \cite{kang17}.

\textbf{8. Reconstruction and extraction:} After convergence, we can:
\begin{itemize}
    \item Apply the final phase corrections to reconstruct an aberration-free image
    \item Extract the object information from the corrected reflection matrix
    \item Recover the PSF profiles by applying the inverse of the corrections to a delta function
\end{itemize}
...
\subsubsection*{Dynamic scattering correction: exchange of roles between the target object and the scattering PSF} 
In the dynamic scattering scenario considered in this work, the fundamental imaging equation (Eq. XX) changes. Instead of considering the static scattering medium (PSF) and $M$ varying illuminations of the conventional (or CTR) reflection matrix acquisition, we consider a static object and varying PSFs due to the dynamic scattering medium. The imaging equation in this case becomes:

\begin{equation}
I_m(\vec{r}) = P_m(\vec{r}) * O(\vec{r}) = O(\vec{r}) * P_m(\vec{r})
\end{equation}

Where $P_m$ is the PSF for the $m$-th realization of the dynamic scatterer. The crucial insight presented in our work is that due to the commutativity of convolution, this equation has exactly the same mathematical form as the random illumination case of conventional reflection matrix acquisition, but with the roles of the object and PSF interchanged.
...
Due to this mathematical equivalence of the imaging equation between the dynamic scattering and random illumination scenarios (Supplementary Eqs. XX and YY) , the covariance matrix of the acquired dataset 'measurement matrix' $\textbf{A}$ has the same TDT structure as that of the covariance matrix of CTR-CLASS, and thus the I-CLASS algorithm can be used to reconstruct the object by naively applying it on the covariance matrix.

\subsubsection*{I-CLASS algorithm}

The I-CLASS algorithm \cite{weinberg2024noninvasive} extends the CLASS algorithm \cite{choi2022flexible} estimation of phase distortions to allow both phase and amplitude distortions in conventional reflection matrix measruements. 
...
In the conventional case of a static medium using multiple illuminations \cite{weinberg2024noninvasive}, %after applying both phase and amplitude corrections, 
the I-CLASS algorithm produces an estimate of the Fourier phase and amplitude of the scattering PSF. However, in the case of a dynamic scattering medium considered in this work, applying the I-CLASS algorithm (without any change on the data or the algorithm) produces the Fourier phase and amplitude of the target object. All that is left is to Fourier transform (or digitally propagate) the algorithm output to reconstruct the image of the object at the target plane. %This role reversal is the key insight that allows us to apply the same algorithm to dynamic scattering media.
    
\end{quote}

We structured this section to gradually introduce the concepts, starting from the basic principles of reflection matrices and progress to our specific implementation for dynamic scattering.

We believe these additions significantly improve the paper's accessibility and provide readers with all necessary background to understand and implement our method. We would be happy to add any additional information or clarifications.
...
\begin{ourresponse}
    ...
    \begin{quote}
        "While deterministic bispectrum reconstruction can address the convergence challenge of phase retrieval, it still requires averaging a large number of speckle grains, limiting the reconstruction to rel;atively simple objects."
    \end{quote}
\end{ourresponse}
...
\begin{ourresponse}
    ...
    We agree that the initial results presented in Figures 2 and 3 require illumination from behind the sample, which makes the presented experiments invasive. However, it is worth noting that the addition of a scattering medium between the light source and the target in these experiments would not change the imaging performance as long as sufficiently high intensity is passed through such scattering medium, as the incoherent imaging configuration only requires homogeneous illumination of the target. %this illumination geometry is not fundamentally different from standard practices in incoherent imaging. For instance, passive illumination sources like sunlight routinely provide constant, uniform illumination from behind a sample – a technique widely accepted in microscopy and imaging sciences.
    
    To clarify this explicitly in the revised manuscript, we added the following note:
    \begin{quote}
        "Note that %The matrix-based imaging approach demonstrated here highlights the fundamental principle of imaging through dynamic scattering media. 
        although these initial experiments utilize a transmission geometry with illumination from behind the target, the addition of a scattering medium between the light source and the target in these experiments would not change the imaging performance as long as a sufficiently high intensity is passed through the scattering medium, as the incoherent imaging configuration only requires homogeneous illumination of the target. Aditionally, our subsequent experimental demonstrations in epi-illumination and detection in fluorescence microscopy (Fig.4) and coherent holographic imaging Fig.~\ref{fig_5}) demonstrate applicability to noninvasive imaging across diverse optical configurations and imaging modalities."
    \end{quote}
...
\end{solved_reviewercomment}
\begin{ourresponse}
We thank the reviewer for raising this point which was indeed not adequately discussed in the original manuscript. Following the reviewer's comment we have added to the revised manuscript the information on the  specific runtime for each iteration and the total runtime for the presented results on our system. This information was added to the \textbf{Methods} section, and it reads:

\begin{quote}
    The run time of the I-CLASS algorithm on a commercially available GPU (Nvidia RTX4090, 24 GB) was approximately $\sim 8ms$ per iteration for 150 camera frames at a resolution of $300 \times 300$ pixels and around $\sim 70ms$ per iteration for 150 camera frames at a resolution of $850 \times 850$ pixels. XX
\end{quote}
\end{ourresponse}
...
\begin{ourresponse}
    We thank the reviewer for noting this confusion. The reviewer is correct - there was a mistake in our original sketch. It should indeed be a non-polarizing beam splitter (BS), and not a polarizing beam splitter (PBS). We have corrected this in the revised manuscript. The corrected Figure and Figure caption now reads:
