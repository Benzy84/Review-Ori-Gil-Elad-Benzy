%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TODO
% After finishing with the paper and supplementary, name all figures as they are in the paper and in the supplementary materials
...
\begin{enumerate}[label=\arabic*.]
    ...
    \end{reviewercomment}
    \begin{ourresponse}
    We agree with the reviewer's comment that in the coherent imaging experiment depicted in Fig. 5, we benefited from using a thin scattering diffuser, enabling us to focus onto a single illumination mode and thus maintain a uniform incident field across all measurements. We fully acknowledge that this requirement of a constant incident wave-field represents a practical limitation in coherent imaging scenarios, particularly when considering realistic, volumetric dynamic scattering media. To clearly address this concern, we have explicitly stated this limitation in our revised manuscript as follows:

    "We note that a constant illumination field is crucial for our coherent imaging approach, a condition easily satisfied by focusing the illumination onto a thin diffuser within a single coherence length. However, in realistic situations, this requirement poses significant practical challenges, as dynamic scattering inherently impacts both the incident and reflected fields."
    
    We further clarify that such limitation does not affect the incoherent imaging scenarios described in our manuscript. In these cases, uniform incoherent illumination naturally arises through temporal or spatial averaging (e.g., by using wide-angle or spatially extended illumination), inherently overcoming the issue of dynamic variations in illumination.
    
    We chose to include a coherent imaging demonstration utilizing a rotating diffuser precisely to illustrate, in a controlled manner, the potential and limitations of matrix-based imaging methods through dynamic scatterers. Additionally, we highlight the applicability of our approach to advanced systems such as flexible multi-core fiber endoscopes (as demonstrated by Choi et al. [Flexible-type ultrathin holographic endoscope for microscopic imaging of unstained biological tissues]), where uniform illumination via single-mode excitation can be straightforwardly achieved.
    \end{ourresponse}
    ...
    We would like to propose an alternative approach that could bridge this gap for coherent imaging through more complex dynamic media. For each static realization of the dynamic scatterer, one could illuminate the object with many ($K >> 1$) different random speckle patterns in rapid succession, sum their \textit{intensities} (not fields) to create one "macro-frame," and only then allow the scatterer to change. similar/like in the incoherent experiment, the incoherent summation would effectively flatten the illumination patterns, so that each macro-frame follows the equation $I_m = P_m * O$, mathematically identical to our incoherent experiments. We could then process these macro-frames with the I-CLASS algorithm to recover the object amplitude (though not phase).

    This hybrid approach offers two significant advantages: (1) it eliminates the need to focus the illumination beam on a small area of the scatterer as required in our current approach, making it applicable to volumetric scattering media, and (2) since the light source remains temporally coherent, coherence-gating could still be used to determine the correct depth prior to reconstruction by analyzing any individual frame before summation. This method could be implemented using a secondary rapidly moving diffuser in the illumination path or a spatial light modulator to generate varied illumination patterns while the primary dynamic scatterer remains temporarily static.

    While this alternative approach could address the limitations identified by the reviewer, we chose our current experimental demonstration with a focused beam on a rotating diffuser as a controlled way to establish the fundamental principles of our matrix-based imaging method through dynamic scatterers. This controlled scenario allowed us to validate the mathematical model and demonstrate proof of concept before addressing more complex imaging scenarios. Additionally, our approach remains directly applicable to certain specialized systems such as flexible multi-core fiber endoscopes (as demonstrated by Choi et al. [Flexible-type ultrathin holographic endoscope for microscopic imaging of unstained biological tissues]), where uniform illumination via single-mode excitation can be straightforwardly achieved.
    \end{ourresponse}
    ...
    \end{reviewercomment}
    \begin{ourresponse}
    We thank the reviewer for noting that three-dimensional scattering can be done via a multi-conjugate strategy for the coherent case. However, these are not directly applicable in the case of incoherent imaging. Following the reviewer's suggestion, we have added the following paragraph to the discussion of the revised manuscript (I suggest adding a paragraph for the discussion similar to the I-CLASS discussion paragraph, the paragraph that starts with:): "Extending the corrected FoV beyond the optical memory effect..."
    \hlred{What about the coherent case? the answer raises this question, a possible answer will be that it will be harder to maintain the  constant illumination constraint (the effective correlation length is small)}
    
    \hlyellow{
    \textbf{Note from Ori:} 
    Great idea and add a small discussion on use of multi-conjugate approaches:
    
    1) multi-conjugate can directly extend the technique to thick \textbf{objects }IN OUR CASE - modeled by by multiple phase and amplitude screens
    
    2) but our thick-medium case is analogous to a thick object in standard CTR-CLASS: is there a solution in standard CTR-CLASS for a thick object?  refer to recent paper by Park et al. where they model the object as a ‘thick’ object and not a planar object. 
    
    3) mention adaptation of gradient-decent model-b ased algorithm [Omri and Jeremy]
    
    this might help in our case since in our case the random medium plays the role of the object for CLASS
    
    possible problem: we do CTR-CLASS so we incohererently average the “object/medium” and I think that in Park’s work it is not the case - might be a serious issue

    \textbf{Discussion:}
    We have demonstrated reconstruction through a thin scattering dynamic layer. Thick scatterers are considered in conventional CLASS or computational wavefront shaping by a multi-conjugate multi-layer scattering profile. However, a thick medium in our case is equicalent to a thick object/target in the conventional case. The modeling and treatement of thick targets proposed in the recent work by SSS et al. [https://www.nature.com/articles/s41467-025-56865-z] may be interesting to study as a possible solution to this challenge.
    }
    ...
    \begin{ourresponse}
        We appreciate the reviewer's important question about the number of required independent frames for reliable estimation. To address this systematically, we conducted a set of numerical simulations investigating the relationship between reconstruction quality, number of realizations (M), and object sparsity. We have added a new supplementary section ("Dependence of Reconstruction Quality on Number of Realizations and Object Sparsity") to the Supplementary Materials that addresses this \hlred{Take from I-CLASS / Acusto-optic the paragraph about the theoretical dependence on M which is logarithmic (from the CTR paper) and add that there are additional factors to this theory such as the object complexity and SNR}.
...
\begin{minipage}{\linewidth}
    ...
    \captionof{figure}{\footnotesize \textbf{Effect of modulation depth on reconstruction quality.} The similarity score shows how reconstruction quality improves with increasing modulation depth. Example reconstructions demonstrate the progressive elimination of background haze as modulation increases, while at higher modulation depths, some haze begins to appear within the object itself. The red circle indicates our chosen modulation depth, which offers a good trade-off in this balance.}
\end{minipage}
...
\end{ourresponse}
    ...
    \hlred{What if you don't know the distance?}
