...
\subsection*{The CLASS algorithm}

The CLASS algorithm \cite{kang17,kang2024implementation} iteratively decomposes the reflection matrix to recover both the object and the phase distortions. Working in Fourier space is advantageous since convolution operations become simpler multiplications.

In real space, we have $\mathbf{R}=\mathbf{P}_{\mathrm{det}}\mathbf{O}\mathbf{P}_{\mathrm{ill}}$, which is a Toeplitz–Diagonal–Toeplitz (TDT) product. After a two-dimensional Fourier transform, this becomes the Diagonal–Toeplitz–Diagonal (DTD) form:
...
\paragraph{Notation.}
Here, $\phi_k$ denotes the illumination phase distortion at pixel $k$, so
$\tilde{\mathbf{P}}_{\mathrm{ill}} = \operatorname{diag}(e^{i\phi_1},e^{i\phi_2},\dots)$;
...
To illustrate the core idea of CLASS, we present a simple $3\times3$ toy example before extending to the full $N\times N$ case:
...
\tilde{\mathbf{O}}
  =\begin{bmatrix}
   ...
   \end{bmatrix},
\]
and $\tilde{\mathbf{P}}_{\mathrm{det}}$ has diagonal elements
$\{e^{i\psi_1},e^{i\psi_2},e^{i\psi_3}\}$.

\textbf{2. Alternating correction approach.} 
The key insight of CLASS is to alternately correct the illumination and detection distortions. When correcting one distortion, we temporarily ignore the other. For instance, to correct $\tilde{\mathbf{P}}_{ill}$, we conceptually treat $\tilde{\mathbf{R}}$ as if it were simply $\tilde{\mathbf{O}}\tilde{\mathbf{P}}_{ill}$. In our example, this product is:
\begin{equation}
...
The critical step in CLASS is to recognize that, due to the Toeplitz structure of $\tilde{\mathbf{O}}$, we can shift the columns of the matrix to align the object components. Specifically, we shift each column so that each row contains the same object component multiplied by different phase factors:
\begin{equation}
...
This alignment is the key that allows us to extract the phase distortions.

\textbf{4. Extracting phase information.} 
To extract the phase distortions, we perform two operations:

a) Calculate the mean of each row to obtain a vector $\vec{T}$:
\begin{equation}
\vec{T} = 
\begin{bmatrix}
O_1 \cdot \frac{1}{3}(e^{i\phi_1} + e^{i\phi_2} + e^{i\phi_3}) \\
O_2 \cdot \frac{1}{3}(e^{i\phi_1} + e^{i\phi_2} + e^{i\phi_3}) \\
O_3 \cdot \frac{1}{3}(e^{i\phi_1} + e^{i\phi_2} + e^{i\phi_3})
\end{bmatrix}
= 
\begin{bmatrix}
O_1 M \\
...
b) Multiply the transpose of the shifted matrix by the conjugate of $\vec{T}$:
\begin{equation}
...
\end{bmatrix}
\end{equation}
where $S = |O_1|^2 + |O_2|^2 + |O_3|^2 > 0$ is a positive real scalar and $\theta_M$ is the phase of $M$.
...
\textbf{5. Applying illumination correction.} 
We create a correction matrix using the extracted phase information:
\begin{equation}
\tilde{\mathbf{P}}_{ill}^{\text{correction}} = \text{diag}(e^{-i\arg(v_1)}, e^{-i\arg(v_2)}, e^{-i\arg(v_3)})
\end{equation}

Applying this correction to the reflection matrix:
\begin{equation}
\tilde{\mathbf{R}}_{\text{corrected}} = \tilde{\mathbf{R}} \cdot \tilde{\mathbf{P}}_{ill}^{\text{correction}}
\end{equation}

\textbf{6. Detection distortion correction.} 
After correcting the illumination distortion, we switch focus to the detection distortion by transposing the matrix:
\begin{equation}
\tilde{\mathbf{R}}^T = \tilde{\mathbf{P}}_{ill}^T \tilde{\mathbf{O}}^T \tilde{\mathbf{P}}_{det}^T  
\end{equation}

With this transposition, $\tilde{\mathbf{P}}_{det}^T$ now plays the role previously held by $\tilde{\mathbf{P}}_{ill}$, allowing us to apply the same procedure to correct the detection distortion.

\textbf{7. Iterative refinement.} 
The illumination and detection correction steps are alternated iteratively until the phase estimates converge. Convergence of this process under single-scattering conditions is proven in \cite{kang17}.

\textbf{8. Reconstruction and extraction.} 
After convergence, we can:
\begin{itemize}
    \item Apply the final phase corrections to obtain an aberration-free image
    \item Extract the object information from the corrected reflection matrix
    \item Recover the PSF profiles by applying the inverse corrections to a delta function
\end{itemize}
...
In the dynamic scattering scenario considered in this work, the fundamental imaging equation (Eq. ~\ref{eq: fund. imaging}) changes. Instead of considering the static scattering medium (PSF) and $m=1..M$ varying illuminations of the conventional reflection matrix acquisition, which can be described by: 
...
\begin{ourresponse}
    We thank the reviewer for this raising this point, which was perhaps not properly explained in the original manuscript. Since the I-CLASS algorithm reconstructs the complex field at the scattering layer plane, we can  digitially back-propagate it to any desired plane. 
    ...
    Importanly, it eliminates the need for precise a priori knowledge of the object-diffuser distance during data acquisition or during I-CLASS iterations, and can allow imaging multiple planes, as was demonstrated in previous works (e.g. Haim et al. Nature Photonics, 2024). 
    %As we now explain in detail in the Methods section titled "Fresnel Propagation via Fourier-Domain Transfer Function," we use the Fresnel propagation transfer function to computationally propagate the reconstructed field from the diffuser plane to any desired plane, including the object plane.
...
%    This numerical propagation capability allows us to determine the correct object plane computationally by finding the propagation distance at which the target features achieve optimal focus—eliminating the need for precise a priori knowledge of the object-diffuser distance during data acquisition. This is analogous to the focusing capability in digital holographic microscopy but applied to imaging through scattering media.
    ...
    To address this point in the revised manuscript, we have added a new supplementary section titled "Digital Autofocus by Fresnel Propagation" that explains this capability and includes a ne figure presenting reconstructions in out of focus planes .
...
\end{solved_reviewercomment}
\begin{ourresponse}
We thank the reviewer for raising this question that was not expelained in detail in the original manuscript.
...
To address this question comprehensively, we have added a new section ("Effect of illumination spot size on coherent imaging through dynamic scattering") to the supplementary material with in depth explanations and a set of numerical simulations that demonstrate and study the limitations of  this effect. For convenience, we reproduce the full section below:
\begin{quote}
    \section*{Supplementary Section SXX: Effect of illumination spot size and homogeneity on coherent imaging through dynamic scattering}

In our coherent imaging demonstration (Fig.~\ref{fig_5} of the main text), maintaining a constant homogeneous illumination pattern at the object plane despite the dynamic scattering introduced by the rotating diffuser is important to ensure the 'fixed object' assumption of our model (Eqs. 6-7 of the main text). %, our method requires that the illumination field $E_m^{ill}(\vec{r})$ remains effectively constant across different acquisitions for our method to work properly.
While obtaining a constant homogeneous illumination through a dynamic scatterer is rather straightforward with spatially-incoherent illumination, it is often challenging when spatially coherent illumination is considerede. Nonetheless, it can be achieved for the case of a dynamic \textit{thin} scatterer, such as a diffuser, by focusing the illumination spot size on the diffuser, such that the spot size is  sufficiently smaller than the diffuser's coherence (or correlation) area. In this case, the illumination beam effectively experiences propagation through a single coherence area with a nearly constant phase function, and thus does not experiences scattering.
Fig.~\ref{fig_S7} demonstrates and numerically studies the limitation of this approach. The top row (Fig.~\ref{fig_S7}a-c) shows the phase pattern of a thin scattering layer with the illumination spot superimposed for three cases: where the illumination spot diameter is 0.1, 1, and 10 times the diffuser correlation length ($d_{\text{correlation}}$), respectively. The second row (Fig.~\ref{fig_S7}d-f) shows the resulting illumination intensity pattern at the object plane after the light has propagated through the diffuser, simulated using Fourier transform propagation.
The third row (Fig.~\ref{fig_S7}g-i) shows the effect of these illumination patterns on the effective object (the product of the object and the illumination), while the fourth row (Fig.~\ref{fig_S7}j-l) shows the reconstruction results. 
