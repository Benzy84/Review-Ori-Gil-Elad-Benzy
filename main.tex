...
\end{ourresponse}
    ...
    \end{reviewercomment}
        ...
        \end{comment}
    \begin{ourresponse}
        We thank the reviewer for this suggestion. We have added a comprehensive description of the I-CLASS algorithm to the Methods section as suggested.
        \begin{quote}
    ...
    \begin{enumerate}[label=3.\alph*.]
        ...
        \begin{ourresponse}
            ...
            \begin{quote}
                ...
                The results of this study are presented in Fig.~\ref{fig_S4}, which displays a side-by-side comparison between the I-CLASS and speckle correlation reconstruction methods for two test objects (Fig.~\ref{fig_S4}a,e). We numerically simulate camera frames under dynamic scattering by Fourier transforming a random phase mask drawn from a uniform distribution over $[0, 2\pi]$ and taking the squared absolute value (Fig.~\ref{fig_S4}b,f). 
                
                For the Cameraman (Fig.~\ref{fig_S4}a), the speckle correlation imaging approach (Fig.~\ref{fig_S4}c), which uses the Hybrid Input-Output (HIO) algorithm \cite{fienup1978reconstruction} for phase retrieval, shows noticeably lower performance compared to I-CLASS reconstruction (Fig.~\ref{fig_S4}d), as indicated by the correlation coefficients between the ground-truth image and the reconstructed images. Similarly, for the USAF resolution target (Fig.~\ref{fig_S4}e), while speckle correlation reconstruction (Fig.~\ref{fig_S4}g) performs somewhat better than with the more complex Cameraman image, the I-CLASS algorithm still demonstrates superior performance (Fig.~\ref{fig_S4}h).
                
                
                Our comparative analysis shows that I-CLASS provides better reconstructions for these test objects than the speckle correlation method.
                

                \begin{minipage}{\linewidth}
                	\centering
                    \includegraphics[width=0.93\linewidth]{figures/figure_S4_for_referee.pdf}
                         \renewcommand{\thefigure}{S4}
                    \captionof{figure}{\footnotesize\textbf{Numerical example imaging through dynamic scattering media.} 
                    Comparison of the I-CLASS and speckle correlation imaging for non-sparse objects. \textbf{a,e} Numerically simulated target objects: the Cameraman image and USAF resolution target. \textbf{b,f} Example camera frames simulated under dynamic scattering. \textbf{c,g} Speckle correlation imaging reconstructions using the HIO algorithm shows limited reconstruction capability, particularly for the more complex Cameraman image. \textbf{d,h} I-CLASS reconstructions demonstrate consistently high fidelity, showing higher correlation coefficients with the ground-truth images for both simulated objects. HIO parameters: $\beta = 1$, 100 realizations, 1000 iterations. Scale bar: 10 px.}
                    \label{fig_S4_2}
                \end{minipage}
