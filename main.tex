...
\end{ourresponse}
    ...
    \begin{ourresponse}
        ...
        \hlblue{for now, this is just a copy from the AO paper, it has to be edited}
        ...
        \begin{quote}
            ...
            %Several key techniques for imaging through scattering layers within the 'memory-effect' region involve measuring the imaging system's transmission matrix. Recent advancements \cite{lee22,weinberg2023noninvasive} propose that, as the distortion caused by illumination does not hold any informative value, it's not necessary to measure the entire large matrix. Instead, one can effectively use a limited number ($M$) of random, uncorrelated illuminations for imaging, rather than analyzing the full matrix.
...
where $O$ is a diagonal matrix containing the propagated object field, and $P$ is a matrix whose columns represent the different diffuser realizations (which are uncorrelated due to the rotation between acquisitions).
        ...
        Still, we emphasize that this measurement choice is largely arbitrary in our case, because once the full complex wavefront is recovered, we can computationally refocus it to any desired depth, as demonstrated by Kwon et al. (Nat. Commun., 2023). This digital propagation capability eliminates the need for precise a priori knowledge of the object-diffuser distance during data acquisition, as we can computationally determine the optimal focus plane during post-processing, as detailed in our new Methods section "Fresnel Propagation via Fourier-Domain Transfer Function" and demonstrated in our new supplementary section "Digital Autofocus through Fresnel Propagation."
    \end{ourresponse}
